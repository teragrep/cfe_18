 ...
 */
public class ProcedureCaptureGroupsTestLinkage extends DBUnitbase {

    public ProcedureCaptureGroupsTestLinkage(String name) {
        super(name);
    }
                ...
                .build(
                        Files
                                .newInputStream(
                                        Paths
                                                .get(
                                                        "src/test/resources/XMLProcedureCaptureGroup/procedureCaptureGroupTestData.xml"
                                                )
                                )
                );
    }
    ...
    public void testProcedureAddCaptureGroupSuccess() throws Exception {
        IDataSet expectedDataSet = new FlatXmlDataSetBuilder()
                .build(
                        new File(
                                "src/test/resources/XMLProcedureCaptureGroup/procedureCaptureGroupTestDataExpectedData1.xml"
                        )
                );
    ...
    public void testNoDuplicateCaptureGroup() throws Exception {
        IDataSet expectedDataSet = new FlatXmlDataSetBuilder()
                .build(
                        new File(
                                "src/test/resources/XMLProcedureCaptureGroup/procedureCaptureGroupTestDataExpectedData2.xml"
                        )
                );
        ITable expectedTable1 = expectedDataSet.getTable("cfe_18.capture_def_group_x_capture_def");
        ITable expectedTable2 = expectedDataSet.getTable("cfe_18.capture_def_group");
