...
(
    ...
    CONSTRAINT ´check_storage_sourcetype´ FOREIGN KEY (storage_id, sourcetype_id) REFERENCES storage_sourcetypes (storage_id, sourcetype_id),
    CONSTRAINT ´check_storage_index´ FOREIGN KEY (storage_id, index_id) REFERENCES storage_indexes (storage_id, index_id),
    UNIQUE (flow_id, storage_id),
    UNIQUE (flow_id, capture_def_id),
    UNIQUE (capture_def_id, flow_id, storage_id),
    start_trxid     BIGINT UNSIGNED GENERATED ALWAYS AS ROW START INVISIBLE,
    end_trxid       BIGINT UNSIGNED GENERATED ALWAYS AS ROW END INVISIBLE,
    PERIOD FOR SYSTEM_TIME(start_trxid, end_trxid)
) WITH SYSTEM VERSIONING;
