...
USE cfe_18;
DELIMITER //
CREATE OR REPLACE PROCEDURE select_all_capture_definitions(metadatakey VARCHAR(255), tx_id INT)
BEGIN
    ...
    IF (metadatakey) IS NULL THEN
        ...
        FROM cfe_18.capture_definition c
                 ...
                 INNER JOIN cfe_18.captureSourcetype FOR SYSTEM_TIME AS OF TRANSACTION @time cs ON c.captureSourcetype_id = cs.id
    ...
    ELSE
        ...
        FROM cfe_18.capture_definition FOR SYSTEM_TIME AS OF TRANSACTION @time c
                 ...
                 INNER JOIN cfe_18.application FOR SYSTEM_TIME AS OF TRANSACTION @time a ON c.application_id = a.id
                 INNER JOIN cfe_18.captureIndex FOR SYSTEM_TIME AS OF TRANSACTION @time ci ON c.captureIndex_id = ci.id
                 INNER JOIN cfe_18.capture_meta FOR SYSTEM_TIME AS OF TRANSACTION @time cm ON c.id = cm.capture_id
                 INNER JOIN cfe_18.capture_meta_key FOR SYSTEM_TIME AS OF TRANSACTION @time cmk ON cm.meta_key_id = cmk.meta_key_id
        WHERE cmk.meta_key_name = metadatakey;
    END IF;
    COMMIT;
END;
//
