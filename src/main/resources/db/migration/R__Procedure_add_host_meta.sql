...
BEGIN
    ...
    if (select id from cfe_18.os_type where os = p_os) is null then
        insert into cfe_18.os_type(os) values (p_os);
        select last_insert_id() into @Os_id;
    else
        select id into @Os_id from cfe_18.os_type where os = p_os;
    end if;

    if (select id from cfe_18.release_version where rel_ver = p_release_ver) is null then
        insert into cfe_18.release_version(rel_ver) values (p_release_ver);
        select last_insert_id() into @RelVer_id;
    else
        select id into @RelVer_id from cfe_18.release_version where rel_ver = p_release_ver;
    end if;

    if (select id from cfe_18.arch_type where arch = p_arch) is null then
        insert into cfe_18.arch_type(arch) values (p_arch);
        select last_insert_id() into @Arch_id;
    else
        select id into @Arch_id from cfe_18.arch_type where arch = p_arch;
    end if;

    if (select id from cfe_18.flavor_type where flavor = p_flavor) is null then
        insert into cfe_18.flavor_type(flavor) values (p_flavor);
        select last_insert_id() into @Flavor_id;
    else
        select id into @Flavor_id from cfe_18.flavor_type where flavor = p_flavor;
    end if;
    ...
    elseif (select count(id)
        ...
        insert into cfe_18.host_meta(arch_id, flavor_id, os_id,
