...
USE cfe_18;
DELIMITER //
CREATE OR REPLACE PROCEDURE select_capture_storages(capture_id INT, tx_id INT)
BEGIN
    ...
    IF ((SELECT COUNT(id)
         FROM cfe_18.capture_def_x_flow_targets FOR SYSTEM_TIME AS OF TRANSACTION @time
         WHERE capture_def_id = capture_id) = 0) THEN
    ...
    ELSE
        ...
        FROM capture_def_x_flow_targets FOR SYSTEM_TIME AS OF TRANSACTION @time cdxft
                 INNER JOIN flow.flow_targets FOR SYSTEM_TIME AS OF TRANSACTION @time ft
                            ON cdxft.flow_target_id = ft.storage_id
