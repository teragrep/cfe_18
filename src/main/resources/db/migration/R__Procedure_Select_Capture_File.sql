...
USE cfe_18;
DELIMITER //
CREATE OR REPLACE PROCEDURE select_cfe_capture(capture_id INT, tx_id INT)
BEGIN
    ...
    WHERE cd.id = capture_id
      AND t.id = cd.tag_id
      AND a.id = cd.application_id
      AND cI.id = cd.captureIndex_id
      AND rT.id = cd.retentionTime_id
      AND cS.id = cd.captureSourcetype_id
      AND c.id = cd.category_id
      AND f.id = cd.flow_id
      AND L7.id = cd.L7_id
      AND ct.id = cd.capture_type_id
      AND cmf.id = ct.id
