...
public class SinkController {
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "201",
                    description = "New capture sink created",
                    content = {
                            @Content(
            ...
            @ApiResponse(
                    responseCode = "409",
                    description = "Flow and protocol combination already exists",
                    content = @Content
            ),
            @ApiResponse(
                    responseCode = "409",
                    description = "Port length exceeded",
                    content = @Content
            ),
            @ApiResponse(
                    responseCode = "400",
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Capture sink retrieved",
                    content = {
                            @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = Sink.class)
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "Sink does not exist",
                    content = @Content
            ),
            @ApiResponse(
                    responseCode = "400",
    ...
    @Operation(
            summary = "Fetch all capture sinks",
            description = "Will return empty list if there are no capture sinks to fetch"
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Capture sink deleted",
                    content = {
                            @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = Sink.class)
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "Capture sink does not exist",
                    content = @Content
            ),
            @ApiResponse(
                    responseCode = "409",
                    description = "Capture sink is being used",
                    content = @Content
            ),
            @ApiResponse(
                    responseCode = "400",
