...
public class CaptureGroupController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Capture linked with group",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = CaptureGroup.class))}),
            @ApiResponse(responseCode = "400", description = "Capture does not exist OR group does not exist  OR type mismatch between capture and group",
                    content = @Content),
            @ApiResponse(responseCode = "500", description = "Internal server error, contact admin", content = @Content)})
    public ResponseEntity<String> createLink(@RequestBody CaptureGroup newCaptureGroup) {
        LOGGER.info("About to insert <[{}]>", newCaptureGroup);
        try {
            CaptureGroup c = captureGroupMapper.createLink(
    ...
    @RequestMapping(path = "/group/captures/{id}", method = RequestMethod.GET, produces = "application/json")
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Capture group does not exist",
                    content = @Content)})
    public ResponseEntity<?> getCaptures(@PathVariable("id") int id, @RequestParam(required = false) Integer version) {
        try {
            List<CaptureGroup> cg = captureGroupMapper.getCaptures(id, version);
            return new ResponseEntity<>(cg, HttpStatus.OK);
        } catch (RuntimeException ex) {
            JSONObject jsonErr = new JSONObject();
            jsonErr.put("message", ex.getCause().getMessage());
    ...
    @RequestMapping(path = "group/{captureId}/{id}", method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Delete capture from group")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Capture deleted from group",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = CaptureGroup.class))}),
            @ApiResponse(responseCode = "400", description = "Capture does not exist OR capture is not linked to group",
                    content = @Content),
            @ApiResponse(responseCode = "500", description = "Internal server error, contact admin", content = @Content)})
    public ResponseEntity<String> deleteLink(@PathVariable("captureId") int captureId,@PathVariable("id") int id) {
        LOGGER.info("Deleting Capture from group <[{}]>", captureId);
        try {
            captureGroupMapper.deleteCapture(captureId,id);
            JSONObject j = new JSONObject();
            j.put("id", captureId);
